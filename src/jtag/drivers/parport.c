...
static int parport_get_inpout_access(void)
{
	...
	if ( dll != NULL ) {
		...
		is_driver_open = (func_is_driver_open)GetProcAddress(dll, "IsInpOutDriverOpen");

		if (out32 == NULL || inp32 == NULL || is_driver_open == NULL) {
			LOG_ERROR("InpOut library seems to be in wrong version. "
					  "Check http://www.highrez.co.uk/Downloads/InpOut32/");
			return -1;
		}
		...
		LOG_ERROR("Could not load InpOut32.dll (32-bit OpenOCD) or InpOutx64.dll (64-bit OpenOCD). "
				  "Check http://www.highrez.co.uk/Downloads/InpOut32/");
		return -1;
	}
...
static int parport_init(void)
{
...
#if PARPORT_USE_GIVEIO == 1
	if (parport_get_giveio_access() != 0 && parport_get_inpout_access() != 0) {
		//if (parport_get_inpout_access() != 0) //as a giveio fallback try inpout32 library
	...
	/* make sure parallel port is in right mode (clear tristate and interrupt */
		...
		(inpout32_lib == 0) ? outb(0x0, parport_port + 2) : out32(parport_port + 2, 0x0);
	#endif
